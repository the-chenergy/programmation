...
void solve() {
    ...
    range(m,t)cin>>tu[t],tu[t]--;
    view(ra,0,n,0,3),view(tu,0,m);
    
    range(m-1,-1,-1,t){
        int b=1E9;
        range(n,i)if(ra[i][tu[t]])amin(b,dp[i]);
        range(n,i)if(!ra[i][tu[t]])dp[i]=1+b;
    }
    int ans=1E9;
    range(n,i)if(ra[i][tu[0]])amin(ans,dp[i]);
    cout<<ans;
}
