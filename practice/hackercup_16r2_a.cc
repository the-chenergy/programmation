...
void solve(int /* case_id */) {
    ...
    string s, t;
    cin >> s >> t;
    vector<array<int, 2>> pre(n + 2), post(n + 2);
    for (int i = 0; i < n; i++) {
        pre[i + 1][0] = s[i] == t[i] ? pre[i][0] : pre[i][1] + 1;
        pre[i + 1][1] = pre[i][1] + (i < n - 1 && t[i] != t[i + 1]);
    }
    for (int i = n - 1; i >= 0; i--) {
        post[i + 1][0] = s[i] == t[i] ? post[i + 2][0] : post[i + 2][1] + 1;
        post[i + 1][1] = post[i + 2][1] + (i > 0 && t[i] != t[i - 1]);
    }
    int ans = 1E9;
    for (int i = 0; i < n; i++)
        ans = min(ans, max(pre[i + 1][0], post[i + 2][0]));
    cout << ans;
}
