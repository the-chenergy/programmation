...
int n,r;
vector<bool>hi,ho;
vector<set<int>>nei,rn;
vi ans;

void find_root(int cur,int par){
    if(~par&&!rn[cur].count(par))r=cur;
    each(rn[cur],nxt)if(nxt!=par){
        find_root(nxt,cur);
    }
}
...
void dfs(int cur,int par){
    ans.push_back(cur);
    each(nei[cur],nxt)if(nxt!=par){
        if(!nei[nxt].count(cur))continue;
        dfs(nxt,cur);
    }
}
...
void solve() {
    ...
    hi.assign(n,0),ho.assign(n,0);
    nei.assign(n,{}),rn.assign(n,{});
    range(n-1){
        char c;
        int a,b;cin>>a>>c>>b;
        nei[a].insert(b),rn[b].insert(a);
        if(c=='?')nei[b].insert(a),rn[a].insert(b);
        else hi[b]=true,ho[a]=true;
    }
    find_root(0,-1);
    // range(n,i){
    //     if(!hi[i]&&ho[i]){
