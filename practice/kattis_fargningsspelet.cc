...
void solve() {
    ...
    while(1){
        ...
        assert(!a[x]);
        a[x]=b[x]=1;
        if(x||m==1)a[(x+n-1)%n]=1;
        if(x<n-1||m==1)a[(x+1)%n]=1;
        trace(x),view(a,0,n),view(b,0,n);
        
        int y=-1,c=999;
        range(n,i)if(!b[i]){
            if(amin(c,1+(i||m==1)*!b[(i+n-1)%n]+(i<n-1||m==1)*!b[(i+1)%n]-4*!a[i])){
                y=i;
            }
        }
        assert(~y);
        cout<<y+1<<"\n";
        a[y]=b[y]=1;
        if(y||m==1)b[(y+n-1)%n]=1;
        if(y<n-1||m==1)b[(y+1)%n]=1;
        trace(y,c),view(a,0,n),view(b,0,n);
    }
}
