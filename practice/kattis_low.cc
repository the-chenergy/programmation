// Solution to Low Power, a problem from the ICPC 2013 World Finals
// (https://open.kattis.com/problems/low)
// #array #binary_search #greedy
//
// This is a fairly straightforward binary search + greedy.
//
// A key fact is that if the minimum difference turns out to be d, then any
// difference > d would also be feasible. This makes the answer
// binary-searchable.
//
// Start with considering the smallest batteries. There's no way to avoid having
// the smallest battery as a power, and there's no reason to avoid having the
// second smallest battery as well, since pairing it with the smallest battery
// will give the smallest difference for one machine. A similar pattern goes on,
// but each time after we finish picking the two smallest batteries for a
// machine, we are now allowed to "skip" 2(k-1) more batteries, where we combine
// them with other smaller batteries so their powers don't matter.
//
...
void solve() {
    ...
    cout << first_where(0, int(1E9), d, works(d));
}
