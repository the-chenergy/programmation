...
const int INF = DEBUG ? 999 : 1E9;
int n, m;
vi mon, ans;
vector<set<pii>> mov;
vector<map<int, int>> adj;

int upd(int cur, int par) {
    if (cur == mon[0]) return INF;
    each(adj[cur], nxt, _) if (nxt != par) amin(ans[cur], upd(nxt, cur) + 1);
    return ans[cur];
}
...
void solve() {
    ...
    if (n < 0) cin >> n;
    cin >> m;
    adj.assign(n, {}), mov.assign(n, {});
    range(n - 1) {
        int a, b;
        cin >> a >> b, a--, b--;
        adj[a][b] = adj[b][a] = 0;
        mov[a].emplace(0, b), mov[b].emplace(0, a);
    }
    mon.assign(m + 1, 0);
    range(1, m + 1, i) cin >> mon[i], mon[i]--;

    ans.assign(n, 0);
    ans[mon[m]] = INF;
    trace(adj, mov, mon, ans);
    range(m - 1, -1, -1, i) {
        int cur = mon[i], nxt = mon[i + 1], aug = INF, u = -343434;
        trace(i, cur, nxt);
        mov[cur].erase({adj[cur][nxt], nxt});
        while (len(mov[cur]) &&
               (tie(aug, u) = *prev(end(mov[cur])), aug > ans[cur] + 1)) {
            mov[cur].erase(prev(end(mov[cur])));
            if (aug != ans[u]) {
                mov[cur].emplace(adj[cur][u] = ans[u], u);
                continue;
            }
            mov[cur].emplace(adj[cur][u] = ans[cur] + 1, u);
            ans[u] = ans[cur] + 1;
            mov[u].erase({adj[u][cur], cur});
            mov[u].emplace(adj[u][cur] = INF, cur);
            trace("    aug", aug, u, mov[cur], mov[u]);
        }
        ans[cur] = INF;
        trace("done aug", i, cur, nxt, mov[cur], ans);
        mov[nxt].erase({adj[nxt][cur], cur});
        aug = INF;
        while (len(mov[nxt]) &&
               (tie(aug, u) = *begin(mov[nxt]), aug != ans[u])) {
            mov[nxt].erase(begin(mov[nxt]));
            mov[nxt].emplace(ans[u], u);
            trace("    esc", aug, u, mov[nxt]);
        }
        ans[nxt] = min(aug + 1, INF);
        mov[nxt].emplace(adj[nxt][cur] = INF, cur);
        mov[cur].emplace(adj[cur][nxt] = ans[nxt], nxt);
        trace("done esc", i, cur, nxt, aug, mov[nxt], ans);
    }
