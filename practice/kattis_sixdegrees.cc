...
void solve(int /* case_id */) {
    ...
    range(m){
        ...
        if(!sti.count(ss))sti[ss]=len(sti),adj.emplace_back();
        if(!sti.count(ts))sti[ts]=len(sti),adj.emplace_back();
        int s=sti[ss],t=sti[ts];
        adj[s].push_back(t),adj[t].push_back(s);
    }
    int remove=0,n=len(sti);
    range(n,s){
        vector<bool>vis(n);
        vis[s]=true;
        int reached=1,step=0;
        deque<int>bfs{s};
        while(len(bfs)){
            step++;
            if(step>6)break;
