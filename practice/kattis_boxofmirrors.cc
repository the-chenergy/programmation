...
int h, w, t[209], ans[109][109];
bool vis[109][109];

void check() {
    range(h + w, si) {
        int y = si < h ? si : h - 1, x = si < h ? 0 : si - h, dy = si >= h,
            dx = si < h;
        while (y >= 0 && x < w) {
            if (ans[y][x]) swap(dy, dx);
            y -= dy, x += dx;
        }
        int ti = y < 0 ? x + h : y;
        if (t[si] != ti) trace("FAIL", si, ti, t[si]), exit(1);
    }
}

bool dfs(int y, int x, int dy, int dx, int ti) {
    trace("  dfs", y, x, dy, dx, ti);
    if (y < 0) trace("    end dfs", y, x, dy, dx, ti, "###", h + x);
    if (y < 0) return h + x == ti;
    if (x >= w) return y == ti;
    if (vis[y][x]) return false;
    vis[y][x] = true;
    if (ans[y][x] >= 0) {
        if (ans[y][x]) swap(dy, dx);
        return dfs(y - dy, x + dx, dy, dx, ti);
    }
    ans[y][x] = 1;
    if (dfs(y - dx, x + dy, dx, dy, ti)) return true;
    ans[y][x] = 0;
    if (dfs(y - dy, x + dx, dy, dx, ti)) return true;
    ans[y][x] = -1;
    return false;
}
...
void solve() {
    ...
    range(h + w, si) {
        int sy = si < h ? si : h - 1, sx = si < h ? 0 : si - h, dy = si >= h,
            dx = si < h;
        memset(vis, false, sizeof(vis));
        dfs(sy, sx, dy, dx, t[si]);
    }

    range(h, y) {
        range(w, x) cout << ans[y][x] << " ";
        cout << "\n";
    }
    check();
}
