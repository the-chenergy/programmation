...
bool check() {
    ...
    int pi = 0, ci = 0;
    while (pi < len(ps)) {
        if (ci < len(cs) - 1 && cs[ci] == 'a' && cs[ci + 1] == 'a')
            aa.push_back(len(t));
        trace("  forward", pi, ci, aa, t);
        if (ci == len(cs) || cs[ci] < ps[pi]) {
            if (aa.empty()) return false;
            while (len(t) > aa.back()) {
                t.pop_back();
                ci--;
                if (len(ta) && ci == ta.back()) {
                    ci--;
                    ta.pop_back();
                }
                pi--;
                trace("      pop", pi, ci, aa.back(), t, ta);
            }
            aa.pop_back();
            t.push_back('~');
            ta.push_back(ci);
            trace("    tilde", pi, ci, aa, t, ta);
            ci++;
        } else {
            t.push_back(cs[ci]);
        }
        ci++;
        if (t.back() > ps[pi]) {
            // t.insert(end(t), begin(cs) + ci, end(cs));
            break;
        }
        pi++;
    }
    ps = move(t);
    ps.insert(end(ps), begin(cs) + ci, end(cs));
    trace("  done", ci, ps);
    return true;
}
...
void solve() {
    ...
    local if (ea = -1, n < 0) n *= -1, cin >> ea;
    trace(n, ea);
    cin >> ps;
    transform(all(ps), begin(ps), ::tolower);
    range(n - 1) {
        cin >> cs;
        transform(all(cs), begin(cs), ::tolower);
        bool ok = check();
        local if (ea >= 0 && !ok) assert(!ea);
        if (!ok) {
            cout << "no";
            return;
        }
    }
    local if (ea >= 0) assert(ea);
    cout << "yes";
}
...
void solve_suite() {
    ...
    for (case_id = 1; case_id <= num_cases; case_id++) {
        local cerr.clear(case_id == 1 ? _S_goodbit : _S_failbit);
