...
using Rock = vector<vector<bool>>;
vector<Rock*> rocks = {
    new Rock{{true, true, true, true}},
    new Rock{{false, true, false}, {true, true, true}, {false, true, false}},
    new Rock{{true, true, true}, {false, false, true}, {false, false, true}},
    new Rock{{true}, {true}, {true}, {true}},
    new Rock{{true, true}, {true, true}},
};
int ri = -1;
...
const int w = 7;
vector<bitset<w>> board;

void print_board() {
    range(len(board) - 1, -1, -1, y) {
        range(w, x) cerr << (board[y][x] ? '@' : '.');
        cerr << "  " << y << "\n";
    }
    cerr << "\n";
}
...
void drop() {
    Rock* rock = rocks[(ri += 1) %= len(rocks)];
    int rh = len(*rock), rw = len(rock->at(0));

    auto valid = [&](int ny, int nx) -> bool {
        if (ny < 0 || nx < 0 || nx + rw > w) return false;
        range(rh, ry) {
            if (ny + ry >= len(board)) break;
            range(rw, rx) {
                if ((*rock)[ry][rx] && board[ny + ry][nx + rx]) return false;
            }
        }
        return true;
    };

    int y = len(board) + 3, x = 2;
    while (1) {
        int dx = moves[(mi += 1) %= len(moves)];
        if (valid(y, x + dx)) x += dx;
        if (!valid(y - 1, x)) break;
        y--;
    }

    board.resize(max(len(board), y + rh));
    range(rh, ry) range(rw, rx) {
        if ((*rock)[ry][rx]) board[y + ry][x + rx] = (*rock)[ry][rx];
    }
}

vector<vector<vi>> times;
vector<int> hs{0};

void solve() {
    string s;
    cin >> s;
    each(s, c) moves.push_back(c == '<' ? -1 : 1);

    // Is this a periodic behavior??
    times.assign(len(rocks), vector(len(moves), vector<int>()));
    int ps = 0, pl = 0;
    range(1, 1E9, t) {
        drop();
        hs.push_back(len(board));
        if (len(times[ri][mi]) == 840) {
            ps = times[ri][mi][420], pl = t - times[ri][mi][420];
            break;
        }
        times[ri][mi].push_back(t);
    }
    trace(ps, pl);

    ll tar = 1000000000000LL;
    cout << hs[ps] + (tar - ps) / pl * (hs[ps + pl] - hs[ps]) +
                hs[ps + (tar - ps) % pl] - hs[ps]
         << "\n";
}
