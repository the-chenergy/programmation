...
map<int, deque<ll>> items;
map<int, array<ll, 3>> op;  // new = old**x[0] * x[1] + x[2]
map<int, tuple<ll, int, int>> test;
ll mod = 1;

void read() {
    string $;
    int i_mon;
    cin >> $ >> i_mon >> $;  // Monkey <i_mon>:
    if (cin.fail()) return;
    ...
    while (1) {
        string xs;
        cin >> xs;  // <x>[,]
        ll x = stoll(xs.back() == ',' ? xs.substr(0, len(xs) - 1) : xs);
        items[i_mon].push_back(x);
        if (xs.back() != ',') break;
    }

    string o, r;
    cin >> $ >> $ >> $ >> $ >> o >> r;  // Operation: new = old <o> <r>
    op[i_mon] = o == "+"     ? array{1LL, 1LL, stoll(r)}
                : r == "old" ? array{2LL, 1LL, 0LL}
                             : array{1LL, stoll(r), 0LL};

    ll d;
    cin >> $ >> $ >> $ >> d;  // Test: divisible by <d>
    int t, f;
    cin >> $ >> $ >> $ >> $ >> $ >> t >> $ >> $ >> $ >> $ >> $ >> f;  // ...
    test[i_mon] = {d, t, f};
    mod *= d;
...
void run_round() {
    each(items, i_mon, xs) {
        ...
        while (len(xs)) {
            ...
            if (p == 2) (x *= x) %= mod;
            (((x *= m) %= mod) += a) %= mod;
            auto [d, t, f] = test[i_mon];
            items[x % d == 0 ? t : f].push_back(x);
        }
    }
}
...
void solve() {
    read();
    range(10'000) run_round();

    vector<int> c;
    each(cts, i, x) c.push_back(x);
