...
int h, w;
vector<vector<int>> g;
vector<vector<int>> max_t, max_b, max_l, max_r;

void populate(vector<vector<int>>& dp, int sy, int sx, int dy, int dx, int py,
              int px) {
    dp.assign(h + 2, vector(w + 2, -1));
    for (int y = sy; y >= 0 && y < h; y += dy)
        for (int x = sx; x >= 0 && x < w; x += dx)
            dp[y + 1][x + 1] = max(dp[y + py + 1][x + px + 1], g[y][x]);
}
...
void solve() {
    while (1) {
        ...
        if (cin.fail()) break;
        g.push_back({});
        each(row, c) g.back().push_back(c - '0');
    }
    h = len(g), w = len(g[0]);

    populate(max_t, 0, 0, 1, 1, -1, 0);
    populate(max_l, 0, 0, 1, 1, 0, -1);
    populate(max_b, h - 1, w - 1, -1, -1, 1, 0);
    populate(max_r, h - 1, w - 1, -1, -1, 0, 1);

    int ans = 0;
    range(h, y) range(w, x) {
        if (g[y][x] > max_t[y][x + 1] || g[y][x] > max_l[y + 1][x] ||
            g[y][x] > max_b[y + 2][x + 1] || g[y][x] > max_r[y + 1][x + 2])
            ans++;
    }
    cout << ans << "\n";
}
