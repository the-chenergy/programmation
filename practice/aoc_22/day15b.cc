...
const int mxy = 4E6;
set<int> tb[mxy + 1];
map<int, int> ps[mxy + 1];

void solve() {
    while (1) {
        int sx, sy, bx, by;
        int r = scanf(" Sensor at x=%i, y=%i: closest beacon is at x=%i, y=%i",
                      &sx, &sy, &bx, &by);
        if (r != 4) break;
        if (by >= 0 && by <= mxy) tb[by].insert(bx);
        int mxd = abs(sx - bx) + abs(sy - by);
        range(max(sy - mxd, 0), min(sy + mxd, mxy) + 1, y) {
            int d = mxd - abs(y - sy);
            ps[y][sx - d]++;
            if (!ps[y][sx - d]) ps[y].erase(sx - d);
            ps[y][sx + d + 1]--;
            if (!ps[y][sx + d + 1]) ps[y].erase(sx + d + 1);
        }
    }
    // trace(ps);
    range(mxy + 1, y) {
