...
set<array<int, 2>> vis = {{0, 0}};

void solve() {
    array<complex<int>, 10> knots;
    while (1) {
        char d;
        int s;
        cin >> d >> s;
        ...
        range(s) {
            knots[0] += dirs[d];
            range(1, len(knots), i) {
                if (abs(knots[i] - knots[i - 1]) > 1) {
                    auto dx = knots[i - 1] - knots[i];
                    knots[i] += complex(min(max(dx.real(), -1), 1),
                                        min(max(dx.imag(), -1), 1));
                }
            }
            vis.insert({knots.back().real(), knots.back().imag()});
        }
    }
    cout << len(vis) << "\n";
}
