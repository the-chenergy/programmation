...
int near[8][3] = {
    {-1, -1, 0b0101}, {-1, 0, 0b0001}, {-1, 1, 0b1001}, {0, -1, 0b0100},
    {0, 1, 0b1000},   {1, -1, 0b0110}, {1, 0, 0b0010},  {1, 1, 0b1010},
};
...
map<int, set<int>> g;
int on, n, lx, hx, ly, hy;

void read() {
    hy = -1;
    for (string row; getline(cin, row);) {
        hy++;
        enumerate(row, x, c) if (c == '#') g[hy].insert(x), amax(hx, x), on++;
    }
}

void add(int y, int x) {
    g[y].insert(x), amin(lx, x), amin(ly, y), amax(hx, x), amax(hy, y), n++;
}
...
void pg() {
    debug {
        range(ly, hy + 1, y) {
            ...
            range(lx, hx + 1, x) cerr << (it->second.count(x) ? '*' : '.');
            cerr << "\n";
        }
        cerr << "\n";
    }
...
void go(int rid) {
    map<int, map<int, int>> ng;
    vector<array<int, 4>> nxt;
    each(g, y, row) each(row, x) {
        int has = 0;
        each(near, dy, dx, ds) if ((has & ds) != ds) {
            if (auto it = g.find(y + dy);
                it != end(g) && it->second.count(x + dx))
                has |= ds;
        }
        int ty = y, tx = x;
        if (has) {
        ...
        nxt.push_back({y, x, ty, tx});
    }

    g.clear(), lx = ly = 1E9, hx = hy = -1E9, n = 0;
    each(nxt, y, x, ty, tx) {
        if (auto it = ng.find(ty); it != end(ng)) {
            if (auto it2 = it->second.find(tx);
                it2 != end(it->second) && it2->second == 1) {
                add(ty, tx);
                continue;
            }
        }
