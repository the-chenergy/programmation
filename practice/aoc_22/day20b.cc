...
const ll dk = 811589153;
vector<ll> a;
vector<int> ai;
int n;
ordered_set<tuple<ld, ll, int>> oa;

void read() {
    ll x;
    while (cin >> x) a.push_back(x * dk), ai.push_back(len(ai));
    n = len(a);
    trace(find(all(a), 0) - begin(a));
}
...
void solve() {
    ...
    range(10, t) {
        oa.clear();
        enumerate(a, i, x) oa.insert({ai[i], x, i});
        trace(t, oa);
        enumerate(a, i, x) {
            if (!x) continue;
            oa.erase({ai[i], x, i});
            ll j = oa.order_of_key({ai[i], x, i});
            // trace(i, x, oa, j, j + x, *oa.find_by_order(j));
            trace("  ", j);
            (j += x - 1) %= n - 1;
            j < 0 && (j += n - 1);
            trace(ai[i], x, j, oa.order_of_key({ai[i], x, i}));
            auto it = oa.find_by_order(j);
            oa.insert(
                {.5 * (get<0>(*it) + (next(it) == end(oa) ? get<0>(*it) + 1
                                                          : get<0>(*next(it)))),
                 x, i});
            trace(oa);
        }
        enumerate(oa, i, _, x, oi) ai[oi] = i;
        // trace(t, n, oa, ai);
    }

    a.clear();
    each(oa, i, x, _) a.push_back(x);
    int z = int(find(all(a), 0) - begin(a));
    ll ans = a[(z + 1000) % n] + a[(z + 2000) % n] + a[(z + 3000) % n];
    cout << ans << "\n";
}
