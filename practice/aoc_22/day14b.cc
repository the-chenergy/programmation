...
void read() {
    ...
    while (1) {
        ...
        cin >> c$ >> y;
        if (sy >= 0) {
            range(min(sy, y), max(sy, y) + 1, cy) {
                if (cy + 1 >= len(g)) g.resize(cy + 2);
                trace(cy, len(g));
                range(min(sx, x), max(sx, x) + 1, cx) g[cy][cx] = true;
            }
        }
        sy = y, sx = x;
    }
...
void print_board() {
    range(fy, min(fy + 16, len(g)), y) {
        range(fx - 40, fx + 40, x) cerr << (g[y][x] ? '#' : '.');
        cerr << "\n";
    }
    cerr << "\n";
}
