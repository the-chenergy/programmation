...
void bp() {
    if (scanf(" Blueprint %i: Each ore robot costs %i ore. Each clay robot "
              "costs %i ore. Each obsidian robot costs %i ore and %i clay. "
              "Each geode robot costs %i ore and %i obsidian.",
              &bid, &u, &v, &w, &x, &y, &z) < 0)
        return;
    trace(bid, u, v, w, x, y, z);

    deque<array<int, 8>> bfs{{1}}, nb;
    int mxh = 0;
    range(mxt, t) {
        stable_sort(all(bfs), comparing(state, -state[6] * 4200 -
                                                   state[4] * 42 - state[2]));
        int l = len(bfs);
        nb = move(bfs);
        range(t < 18 ? l : min(l / 2, 2000000)) {
            auto [a, b, c, d, e, f, g, h] = nb.front();
            nb.pop_front();
            amax(mxh, h);
            if (t < 5 || !(rand() % l / 6)) trace(t, l, a, b, c, d, e, f, g, h);
            if (b >= y && f >= z)
                bfs.push_back(
                    {a, b - y + a, c, d + c, e, f - z + e, g + 1, h + g});
            if (b >= w && d >= x)
                bfs.push_back(
                    {a, b - w + a, c, d - x + c, e + 1, f + e, g, h + g});
            if (b >= v)
                bfs.push_back({a, b - v + a, c + 1, d + c, e, f + e, g, h + g});
            if (b >= u)
                bfs.push_back({a + 1, b - u + a, c, d + c, e, f + e, g, h + g});
            bfs.push_back({a, b + a, c, d + c, e, f + e, g, h + g});
        }
    }
    ans += ll(bid) * mxh, trace(bid, mxh);
