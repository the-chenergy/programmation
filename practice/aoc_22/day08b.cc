...
void score_rows(int sx, int dx) {
    range(h, y) {
        deque<array<int, 2>> tall = {{int(1E9), sx}};
        for (int x = sx; x >= 0 && x < w; x += dx) {
            while (tall.back()[0] < g[y][x]) tall.pop_back();
            score[y][x] *= abs(x - tall.back()[1]);
            tall.push_back({g[y][x], x});
        }
    }
}

void score_cols(int sy, int dy) {
    range(w, x) {
        deque<array<int, 2>> tall = {{int(1E9), sy}};
        for (int y = sy; y >= 0 && y < h; y += dy) {
            while (tall.back()[0] < g[y][x]) tall.pop_back();
            score[y][x] *= abs(y - tall.back()[1]);
            tall.push_back({g[y][x], y});
        }
    }
}
...
void solve() {
    ...
    ll ans = 0;
    range(h, y) range(w, x) ans = max(ans, score[y][x]);
    cout << ans << "\n";
}
