...
map<string, int> mid;
map<int, string> num;
map<int, tuple<int, char, int>> dep;

int get_mid(string nam) {
    if (auto it = mid.find(nam); it != end(mid)) return it->second;
    return mid[nam] = len(mid);
}
...
void read() {
    ...
    if (isdigit(x[0])) {
        ...
        cin >> op >> y;
        int xi = get_mid(x), yi = get_mid(y);
        dep[si] = {xi, op, yi};
    }

    if (s == "humn") dep.erase(si), num[si] = "x";

    read();
}

string eva(int m);
string ari(int x, char op, int y) {
    return "(" + eva(x) + string(1, op) + eva(y) + ")";
}

string eva(int m) {
    if (auto it = num.find(m); it != end(num)) return it->second;
    auto [x, op, y] = dep[m];
    return ari(x, op, y);
}
...
void solve() {
    ...
    auto [x, _, y] = dep[mid["root"]];
    cout << eva(x) << " = " << eva(y) << "\n";
    // Pipe this output into the M A G I C Python solver
}
