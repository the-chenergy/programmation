...
void read() {
    char us[22];
    int r;
    if (scanf(" Valve %s has flow rate=%i;", us, &r) < 0) return;
    if (!nam.count(us)) nam[us] = len(nam), rat.push_back(0), adj.push_back({});
    int u = nam[us];
    rat[u] = r;
    string _;
    cin >> _ >> _ >> _ >> _;  // tunnels lead to valves
    while (1) {
        string vs;
        cin >> vs;
        bool fin = vs.back() != ',';
        if (!fin) vs.pop_back();
        if (!nam.count(vs))
            nam[vs] = len(nam), rat.push_back(0), adj.push_back({});
        int v = nam[vs];
        adj[u].push_back(v);
        if (fin) break;
    }
...
void solve() {
    ...
    n = len(nam);
    ip.assign(n, -1);
    range(n, u) if (rat[u]) pi.push_back(u), ip[u] = len(pi) - 1;
    k = len(pi);

    dp.assign(mxt + 1, vector(1 << k, vi(n, 0)));
    range(1, mxt + 1, t) range((1 << k) - 1, -1, -1, p) range(n, u) {
        each(adj[u], v) {
            amax(dp[t][p][u], dp[t - 1][p][v]);
            if (t >= 2 && ip[u] >= 0 && !(p & (1 << ip[u]))) {
                amax(dp[t][p][u],
                     rat[u] * (t - 1) + dp[t - 2][p | (1 << ip[u])][v]);
            }
        }
    }
    int ans = 0, s = nam["AA"];
    range(1 << k, p) {
        amax(ans, dp[mxt][p][s] + dp[mxt][((1 << k) - 1) ^ p][s]);
    }
    cout << ans << "\n";
}
