...
void solve() {
    read();
    enumerate(a, i, x) oa.insert({i, x});
    trace(oa);

    enumerate(a, i, x) {
        if (!x) continue;
        oa.erase({i, x});
        int j = int(oa.order_of_key({i, x}));
        trace(i, x, oa, j, j + x, *oa.find_by_order(j));
        (j += x - 1 - (x < 0)) %= n - 1;
        j < 0 && (j += n);
        auto it = oa.find_by_order(j);
        oa.insert({.5 * (it->first + (next(it) == end(oa) ? it->first + 1
                                                          : next(it)->first)),
                   x});
        trace(oa);
    }
    ...
    a.clear();
    each(oa, i, x) a.push_back(x);
    int z = int(find(all(a), 0) - begin(a));
    int ans = a[(z + 1000) % n] + a[(z + 2000) % n] + a[(z + 3000) % n];
    cout << ans << "\n";
}
