...
const int l = EXAMPLE ? 4 : 50;
using P = Point3D<double>;

array<int, 2> dirs[]{{0, 1}, {1, 0}, {0, -1}, {-1, 0}};
vector<string> world;
vector<pair<bool, int>> moves;  // {is_turn, ? 1 or -1 : steps}
int h, w;
...
void read_moves() {
    int s;
    cin >> s;
    moves.push_back({false, s});
    char d;
    cin >> d;
    if (!cin) return;
    moves.push_back({true, d == 'R' ? 1 : 3});
    read_moves();
}

P rp(P p) {
    range(3, i) p[i] = abs(p[i]) < .5 ? 0 : round(p[i]);
    return p;
}

pair<P, P> mov(P p, P f) {
    int out = 0, no = 0;
    p = p + f;
    range(3, i) if (p[i] < -l || p[i] > l) out |= 1 << i, no++;
    assert(no == 1 || no == 2);
    if (no == 2) {
        P ax;
        ax[__lg(7 ^ out)] = 1;
        // I'm done with math for the year. Trying both directions here.
        auto f1 = f.rotate(M_PI_2, ax), f2 = f.rotate(-M_PI_2, ax);
        int f1notok = 0;
        auto p1 = p + f1, p2 = p + f2;
        range(3, i) if (p1[i] < -l || p1[i] > l) f1notok++;
        if (f1notok == 2)
            p = p2, f = f2;
        else
            p = p1, f = f1;
...
P turn(P p, P f, int d) {
    if (d % 4 == 0) return f;
    int t = -1;
    range(3, i) if (p[i] < -l || p[i] > l) assert(!~t || trace(p, f, d)), t = i;
    assert(~t);
    P ax;
    ax[t] = 1;
    return rp(f.rotate(M_PI_2 * copysign(1., p[t]) * (4 - d % 4), ax));
}

map<P, pii> c32;
map<pii, P> c23;
const P sp(-l + 1, l + 1, -l + 1), sf(2, 0, 0);

void calc_faces() {
    int sx = 0;
    while (world[0][sx] == ' ') sx++;
    auto vis = vector(h, vector(w, false));
    vis[0][sx] = true;
    deque<tuple<int, int, int, P, P>> dfs{{0, sx, 0, sp, sf}};
    while (len(dfs)) {
        auto [cy, cx, cd, cp, cf] = dfs.back();
        dfs.pop_back();
        // trace("cur", cy, cx, cd, cp, cf);
        c32[cp] = {cy, cx}, c23[{cy, cx}] = cp;
        range(4, dd) {
            int d = (cd + dd) % 4;
            auto [y, x] = dirs[d];
            y += cy, x += cx;
            ...
            P f = turn(cp, cf, dd), p;
            tie(p, f) = mov(cp, f);
            dfs.emplace_back(y, x, d, p, f);
            // trace("  nxt", dd, y, x, d, p, f);
        }
    }
}
...
void solve() {
    ...
    each(moves, is_turn, s) {
        ...
        range(s) {
            tie(np, nf) = mov(p, f);
            auto [y, x] = c32[np];
            // trace("  step", s, p, f, np, nf, y, x, world[y][x]);
            if (world[y][x] == '#') break;
            p = np, f = nf;
        }
    }
    ...
    cout << 1000 * (y + 1) + 4 * (x + 1) << " + ?"
         << "\n";
    // You can figure out the direction for yourself -- I'm done with math IYDK
    trace(p, f, y, x);
}

void solve_suite() {
    freopen(EXAMPLE ? "./sandbox/ix.txt" : "./sandbox/i.txt", "r", stdin);
