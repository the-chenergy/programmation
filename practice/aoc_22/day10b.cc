...
vector<array<int, 2>> vals = {{1, 1}};

int val(int t) {
    return prev(upper_bound(all(vals), array{t, int(1E9)}))->at(1);
}
...
void solve() {
    ...
    vector<string> image(6, string(40, '.'));
    enumerate(image, y, row) {
        enumerate(row, x, px) {
            int i = y * len(row) + x;
            if (abs(i % len(row) - val(i + 1)) <= 1) px = '#';
        }
    }
    each(image, row) cout << row << "\n";
}
