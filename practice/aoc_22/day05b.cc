...
map<string, int> stack_id;    // stack_name -> stack_id
vector<array<int, 3>> moves;  // {count, source_stack, target_stack}

void read_stacks() {
    // Buffer everything before blank line to probe for line with stack names
    vector<string> lines;
    while (1) {
        string s;
        getline(cin, s);
    ...
    // Read stack names (without assuming they're merely numbered 1..N)
    stringstream names_in(lines.back());
    while (1) {
        string s;
        names_in >> s;
        if (names_in.fail()) break;
        stack_id[s] = len(stacks);
        stacks.emplace_back();
    }

    // Parse the stack contents, preserving the stacks' original ordering
    each(lines, line) {
        range(len(stacks), i) {
            if (i * 4 + 1 >= len(line)) break;
...
void read_moves() {
    while (1) {
        ...
        cin >> $ >> ct >> $ >> src >> $ >> tar;
        if (cin.fail()) break;
        moves.push_back({ct, stack_id[src], stack_id[tar]});
    }
}
...
void solve() {
    ...
    each(moves, ct, src, tar) {
        stacks[tar].insert(end(stacks[tar]), end(stacks[src]) - ct,
                           end(stacks[src]));
        stacks[src].erase(end(stacks[src]) - ct, end(stacks[src]));
    }
    string ans;
    each(stacks, s) ans.push_back(s.back());
    cout << ans << "\n";
}
