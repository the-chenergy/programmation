// Solution to Classrooms, a problem from the Hong Kong Regional 2016
// (https://open.kattis.com/problems/classrooms)
// #intervals #greedy
//
// Always try to schedule the activity with the earliest end time first.
...
void solve() {
    ...
    for(int i=0;i<k;i++)r.insert({0,i});
    
    int ans=0;
    for(int i=0;i<n;i++){
        auto[e,s]=ts[i];
        auto it=r.lower_bound({s,-1});
        if(it==begin(r))continue;
        it--;
        int ri=it->second;
        r.erase(it);
        r.insert({e,ri});
