...
map<string, int> mti{{"JAN", 0}, {"FEB", 1}, {"MAR", 2},  {"APR", 3},
                     {"MAY", 4}, {"JUN", 5}, {"JUL", 6},  {"AUG", 7},
                     {"SEP", 8}, {"OCT", 9}, {"NOV", 10}, {"DEC", 11}};
map<string, int> dti{{"FRI", 0}, {"SAT", 1}, {"SUN", 2}, {"MON", 3},
                     {"TUE", 4}, {"WED", 5}, {"THU", 6}};
int mds1[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
int mds2[] = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

int fri(int mds[12], int m, int d, int j1) {
    return !((d + reduce(mds, mds + m, 0) + j1) % 7);
}
...
void solve() {
    ...
    string ms, j1s;
    cin >> d >> ms >> j1s;
    d--, m = mti[ms], j1 = dti[j1s];
    if (fri(mds1, m, d, j1) == fri(mds2, m, d, j1)) {
        cout << (fri(mds1, m, d, j1) ? "TGIF" : ":(");
    } else {
        cout << "not sure";
    }
