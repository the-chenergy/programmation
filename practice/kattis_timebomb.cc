...
void solve() {
    memset(d,-1,sizeof(d));
    d[0b111101101101111]=0;
    d[0b001001001001001]=1;
    d[0b111001111100111]=2;
    d[0b111001111001111]=3;
    d[0b101101111001001]=4;
    d[0b111100111001111]=5;
    d[0b111100111101111]=6;
    d[0b111001001001001]=7;
    d[0b111101111101111]=8;
    d[0b111101111001111]=9;
    // if (0 || cin.fail()) exit(0);
    range(5,i)getline(cin,s[i]);
    trace(s);
    n=(len(s[0])+1)/4;
    ll xx=0;
    range(n,i){
        int yy=0;
        range(5,y)range(3,x){
            trace(i,y,x,i*4+x,y*3+x);
            if(s[y][i*4+x]=='*')yy|=1<<(15-1-(y*3+x));
        }
        trace(bitset<18>(yy));
        if(d[yy]==-1){
            cout<<"BOOM!!";
