...
for s in inp.split("\n\n"):
    a, b, c = s.splitlines()
    ax, ay = map(lambda t: t.split("+")[1], a.split(": ")[1].split(", "))
    bx, by = map(lambda t: t.split("+")[1], b.split(": ")[1].split(", "))
    px, py = map(lambda t: t.split("=")[1], c.split(": ")[1].split(", "))
    A.append(list(map(int, (ax, ay, bx, by, px, py))))
...
def f(i):
    ...
    ans = f(i + 1)
    ax, ay, bx, by, px, py = A[i]

    @cache
    def g(x, y, r, r2):
        if x >= px or y >= py:
            return 0 if x == px and y == py else inf
        ans = inf
        if r:
            ans = 3 + g(x + ax, y + ay, r - 1, r2)
        if r2:
            ans = min(ans, 1 + g(x + bx, y + by, r, r2 - 1))
        return ans

    F = f(i + 1)
    G = g(0, 0, 100, 100)
    if G < inf:
        ans = min(ans, (F[0] - 1, F[1] + G))
