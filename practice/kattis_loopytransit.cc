...
void solve() {
    ...
    range(m) {
        int a, b;
        cin >> a >> b;
        if (case_id == 2) trace(a, b);
        con[a][b] = true;
    }
    if (case_id == 2) view(con, 0, n, 0, n);

    int ans = 0;
    range(1 << n, s) if (__builtin_popcount(s) >= 2) {
        int l = __lg(s);
        vi is;
        range(l, i) if (s & (1 << i)) is.push_back(i);
        do {
            bool ok = true;
            int p = l;
            each(is, i) {
            ...
            ok &= con[is.back()][l];
            ans += ok;
            if (case_id == 2) trace(s, is, l, ok, is.back());
