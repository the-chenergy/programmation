...
// Returns {size, depth}
pair<int, int> rob(int cur, int par, int rem) {
    if (cur && !rem) trace("  robbed", cur);
    if (!cur || !rem) return {0, 0};
    rem--;
    int l = 0, r = 0;
    each(adj[cur], nxt) if (nxt != par) !l ? l = nxt : r = nxt;
    int ls = 0, ld = 1E9, rs = 0, rd = 1E9;
    bool done = false, rob_left = true;
    while (!done) {
        done = true;
        if (rob_left) {
            tie(ls, ld) = rob(l, cur, rem);
            rem = min(rem, ld + 1);
            if (rd > ld + 1) done = false;
        } else {
            tie(rs, rd) = rob(r, cur, rem);
            rem = min(rem, rd + 1);
            if (ld > rd + 1) done = false;
        }
        rob_left ^= 1;
    }
    trace("done", cur, par, ls, rs, ld, rd);
