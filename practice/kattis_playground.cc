...
#define ans(x) return void(cout<<(x?"YES":"NO"))
...
void solve(int /* case_id */) {
    ...
    range(n,i){
        double x;cin>>x;
        a[i]=ll(x*2000+.5);//convert to int diameters
    }
    if(n==1)ans(0);
    if(n==2)ans(a[0]==a[1]);
    sort(a,a+n);
    range(n,b)range(b+3,n+1,e){
        ll tot=reduce(a+b,a+e-1,0LL);
        if(tot>=a[e-1])ans(1);//triangle inequality
    }
    ans(0);
}
