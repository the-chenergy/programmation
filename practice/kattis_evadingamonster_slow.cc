...
void init() {
    ...
    mon.resize(m);
    mon[0] = 0;
    range(1, m, i) cin >> mon[i], mon[i]--;
}

int lca(int u, int v) { return l->lca(u, v); }

int dist(int u, int v) { return dep[u] + dep[v] - 2 * dep[lca(u, v)]; }

// Whether r is on the path from u to v
int blocked(int u, int v, int r) {
    return lca(u, v) == r || ((lca(u, r) == r) ^ (lca(v, r) == r));
}

// Solves group 1 of https://open.kattis.com/problems/evadingamonster
// O(M * N**2 log N) DP
// Could've been O(M * N**2) with a simple DFS at each (i, u) to find dist(u, v)
void solve() {
    init();
    ...
    range(m - 1, -1, -1, i) {
        range(n, u) {
            if (mon[i] == u) continue;
            range(n, v) if (!blocked(u, v, mon[i])) {
                amin(dp[i][u], dist(u, v) + dp[i + 1][v]);
            }
        }
    }
    cout << (dp[0][1] >= 1E9 ? -1 : dp[0][1]);
}

void solve_suite() {
    local freopen("./sandbox/i7.txt", "r", stdin);
    local cout.open("./sandbox/o7e.txt");
    // cin >> num_cases;
