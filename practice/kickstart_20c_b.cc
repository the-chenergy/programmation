// Solution to Stable Wall, problem B from the Google Kick Start 2020 Round C
// (https://codingcompetitions.withgoogle.com/kickstart/round/000000000019ff43/00000000003379bb)
// #graph #topo_sort
//
// Build the graph with edge u->v if character u is right above v in the input.
...
void solve() {
    ...
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++) {
            ...
            nei[g[y][x]];
            if (!y) continue;
            if (g[y][x] != g[y - 1][x]) nei[g[y][x]].insert(g[y - 1][x]);
            nei[g[y - 1][x]];
        }
    for (auto& [u, vs] : nei) {
        ind[u];
