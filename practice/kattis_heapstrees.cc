// Solution to Heaps from Trees, a problem from the NAIPC 2017
// (https://open.kattis.com/problems/heapstrees)
// #graph #dfs #dp #binary_search
//
// If the input tree only had exactly one leaf node, this problem would just
// become the longest increasing subsequence problem (or decreasing in this
// case, but same story). That problem has a straightforward O(n**2) dynamic
// programming solution which can be improved to O(n log n) with the help
// of binary search.
//
// The difference now between the classical LIS problem and this problem is the
// involvement of the tree, or that each element may have more than one
// successor as opposed to the array's case where there is only one next
// element for each element in the array. What you'll notice is that the
// successors do not interfere with each other. So what we can do is after
// running the regular LIS algorithm, simply merge the output sequence from the
// children into a single sorted sequence, keeping all the duplicates. This
// could be implemented with a multiset to avoid copying the sequence from every
// child, which should provide an amortized running time of O(n log n).
...
void dfs(int i,S& s){
    ...
    vector<S>cs(len(chi[i]));
    int mxj=-1,mxl=0;
    for(int j=0;j<len(chi[i]);j++){
        dfs(chi[i][j],cs[j]);
        if(amax(mxl,len(cs[j])))mxj=j;
    }
    s=move(cs[mxj]);
    for(int j=0;j<len(chi[i]);j++){
        if(j==mxj)continue;
    ...
    if(auto it=s.lower_bound(val[i]);it!=end(s))s.erase(it);
    s.insert(val[i]);
}
...
void solve() {
    ...
    if (0 || !cin) exit(0);
    for(int i=0;i<n;i++){
        cin>>val[i]>>par[i],par[i]--;
        if(~par[i])chi[par[i]].push_back(i);
    }
    S s;
    dfs(0,s);
    cout<<len(s);
