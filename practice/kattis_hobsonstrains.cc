...
int n, k, nxt[500009], jump[500009], jump1[500009], ans[500009], cl, tl[500009];
bool cyc[500009];
vector<int> prv[500009];

void find_jumps() {
    memcpy(jump, nxt, sizeof(nxt));
    range(__lg(k) - 1, -1, -1, p) {
        range(n, i) jump1[i] = jump[jump[i]];
        memcpy(jump, jump1, sizeof(jump));
        if ((k >> p) & 1) {
            range(n, i) jump1[i] = nxt[jump[i]];
            memcpy(jump, jump1, sizeof(jump));
        }
    }
}
...
void calc(int u, int d = 0) {
    trace("calc", u, d);
    tl[u] = 1;
    each(prv[u], v) if (!cyc[v]) {
        calc(v, d + 1);
        tl[u] += tl[v];
    }
    ans[u] += tl[u], trace(u, d, tl[u], ans[u]);
    if (k < d + cl) {
        ans[jump[u]] -= tl[u] - 1;
        trace(u, k, d, cl, jump[u], tl[u], ans[jump[u]]);
    }
}
...
void connect(int s) {
    ...
    int ss = 0, f = nxt[s], p;
    range(min(k, cl - 1)) ss += tl[f], f = nxt[f];
    trace(f, ss);
    p = nxt[s], s = f;
    do {
        ans[f] += ss, trace(f, s, p, ss, ans[f]);
        ss -= tl[p], ss += tl[f];
        f = nxt[f], p = nxt[p];
    } while (f != s);
}
...
void solve() {
    ...
    memset(ans, 0, sizeof(ans)), memset(cyc, false, sizeof(cyc));
    range(n, i) if (!ans[i]) connect(i);

    range(n, i) cout << ans[i] << "\n";
}
...
void solve_suite() {
    ...
    for (case_id = 1; case_id <= num_cases; case_id++) {
        local if (!debug_id || case_id == debug_id) _cout.clear(), cerr.clear();
