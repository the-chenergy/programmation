...
def build(s):
    for d in range(10):
        s = s.replace(str(d), f"[{d}]")
    return eval(s)


inp = list(map(build, inp.splitlines()))


def reduce(u):
    def left_most(u):
        return u if len(u) == 1 else left_most(u[0])

    def right_most(u):
        return u if len(u) == 1 else right_most(u[1])

    def explode(d, u, l, r):
        # print(f"explode {d=} {u=}")
        # print(f"\t{l=} {r=}")
        if len(u[0]) == 1 and len(u[1]) == 1 and d >= 4:
            if l:
                l[0] += u[0][0]
            if r:
                r[0] += u[1][0]
            ...
            u.pop()
            return 1
        if len(u[0]) == 2 and explode(d + 1, u[0], l, left_most(u[1])):
            return 1
        if len(u[1]) == 2 and explode(d + 1, u[1], right_most(u[0]), r):
            return 1
        return 0
    ...
    def split(u):
        ...
        if len(u) == 1:
            if u[0] >= 10:
                ...
                u.append([(x + 1) // 2])
                return 1
        elif split(u[0]) or split(u[1]):
            return 1
        return 0

    while 1:
        if explode(0, u, None, None) or split(u):
            # print("\n------\n")
            continue
        break
