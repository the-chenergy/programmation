// Solution to Make It Increasing, problem E from the Educational Codeforces
// Round 97 Division 2
// https://codeforces.com/contest/1437/problem/E
//
// Divide the input array into pieces separated by the fixed points B. Find the
// longest increasing subsequence in each piece that must contain the fixed
// (which are now the piece's start and end) points. Anything not in the LIS
// are the ones we need to change. O(N log N) overall.
//
// One helpful trick to reduce indexing pain: a strictly-increasing array must
// have the property that, if we subtract each of the elements by its index, the
// array becomes non-decreasing (e.g., {3, 4, 5, 1, 2} -> {3, 3, 3, -2, -2}).
// So apply this transformation to A and find the longest non-decreasing
// subsequences instead. This solves the indexing problem where {3, 5, 4} cannot
// be made increasing given the numbers 3 and 4 being fixed, despite having 
// {3, 4} as the LIS. After that transformation {3, 5, 4} becomes {3, 4, 2}
// which is clearly impossible (3 > 2).
...
int n,k,a[500009],s[500009],b[500009],ans,l;
...
void solve() {
    ...
    a[0]=-1E9,a[n+1]=1E9,b[0]=0,b[k+1]=n+1;
    ...
    for(int i=0;i<=k;i++){
        ...
        for(int j=b[i]+1;j<=b[i+1];j++){
            ...
            int t=int(upper_bound(s,s+l,a[j])-s);
            l+=t==l;
            s[t]=a[j];
        }
        ans+=b[i+1]-b[i]+1-l;
