...
int n;
ll ct[3][3],a,b,c,d,x0,y0_,m;
array<int,3>gr[]{{0,0,0},{1,1,1},{2,2,2},{0,1,2}};

void solve() {
    cin>>n>>a>>b>>c>>d>>x0>>y0_>>m;
    // if (0 || !cin) exit(0);
    memset(ct,0,sizeof(ct));
    range(n,i){
        trace(i,x0,y0_,x0%3,y0_%3);
        ct[x0%3][y0_%3]++;
        x0=(a*x0+b)%m;
        y0_=(c*y0_+d)%m;
    }
    trace(ct,ct[0][1]);
    ll ans=0;
    each(gr,xs)each(gr,ys){
        auto[xa,xb,xc]=xs;
        auto[ya,yb,yc]=ys;
        trace(xa,xb,xc,ct);
        if(xa==xb&&xb==xc&&ya==yb&&yb==yc){
            ans+=ct[xa][ya]*(ct[xa][ya]-1)*(ct[xa][ya]-2)/6;
        }else{
            do{
                ya=ys[0],yb=ys[1],yc=ys[2];
                // trace("  ",xa,xb,xc,ya,yb,yc,ct[xa][ya],ct[xb][yb],ct[xc][yc]);
                ans+=ct[xa][ya]*ct[xb][yb]*ct[xc][yc];
            }while((xa!=xb||xb!=xc)&&next_permutation(all(ys)));
        }
    }
    cout<<ans;
}
...
void solve_suite() {
    debug freopen("./sandbox/ix.txt", "r", stdin);
