...
map<int,string>prog;
decltype(prog)::iterator pc;
map<char,int>var;

void read(){
    int l;string s;cin>>l,assert(l>=0||trace(l));
    if(cin.fail())return;
    getline(cin,s);
    prog[l]=s;
    read();
...
int eval(string x){
    assert(len(x)||trace(x));
    if(isupper(x[0])){
        assert(len(x)==1||trace(x));
        return var[x[0]];
    }
    return stoi(x);
}

int alu(string l,string op,string r){
    int x=eval(l),y=eval(r),a=0;
    if(op=="+")__builtin_sadd_overflow(x,y,&a);
    else if(op=="-")__builtin_ssub_overflow(x,y,&a);
    else if(op=="*")__builtin_smul_overflow(x,y,&a);
    else if(op=="/")assert(y||trace(l,op,r,x,y)),a=x/y;
...
void solve() {
    ...
    pc=begin(prog);
    while(pc!=end(prog)){
        stringstream s(pc->second);
        string c;s>>c;
        bool j=false;
        if(c=="LET"){
            string x;s>>x,assert((len(x)==1&&isupper(x[0]))||trace(x));
            string $;s>>$,assert($=="="||trace($));
            string l,op,r;s>>l>>op>>r;
            if(s.fail()){
                var[x[0]]=eval(l);
            }else{
                var[x[0]]=alu(l,op,r);
            ...
            s>>$,assert($=="THEN"||trace($));
            s>>$,assert($=="GOTO"||trace($));
            int g;s>>g,assert(g>=0||trace(g));
            if(b){
                pc=prog.find(g),assert(pc!=end(prog)||trace(g));
                j=true;
            }
        }else{
            assert(c=="PRINT"||c=="PRINTLN"||trace(c));
            string x;
            getline(s,x);
            if(x[0]==' ')x.erase(begin(x));
            if(x[0]=='"'){
                x.erase(begin(x));
                assert(x.back()=='"'||trace(x));
                x.pop_back();
                cout<<x;
            }else{
                cout<<eval(x);
