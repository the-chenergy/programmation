...
void solve() {
    ...
    for(auto cit=begin(rs);cit!=end(rs);cit++){
        for(auto rit=begin(cit->second);rit!=end(cit->second);rit++){
            int lx=cit==begin(rs)?1:cit->first,
                hx=next(cit)==end(rs)?p:next(cit)->first-1,
                ly=rit==begin(cit->second)?1:*rit,
                hy=next(rit)==end(cit->second)?q:*next(rit)-1;
            trace(cit->first,*rit);
            cout<<lx<<" "<<ly<<" "<<hx<<" "<<hy<<"\n";
        }
    }
