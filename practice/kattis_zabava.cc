...
void solve() {
    ...
    //dp[i][r]=min(dp[i+1][r-u]+... for u in [0..r])
    //...->noise level when spreading u removals evenly
    memset(dp,63,sizeof(dp)),memset(dp[m],0,sizeof(dp[m]));
    range(m-1,-1,-1,i)range(k+1,r)range(r+1,u){
        ll x=c[i]/(u+1);
        ll y=(x+1)*(x+2)/2*(c[i]%(u+1))+x*(x+1)/2*(u+1-c[i]%(u+1));
        trace(i,r,u,x,y);
        amin(dp[i][r],y+dp[i+1][r-u]);
    }
    cout<<dp[0][k];
}
