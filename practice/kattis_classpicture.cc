...
void solve() {
    ...
    range(n,i)cin>>names[i];
    sort(names,names+n);
    cin>>m;
    memset(adj,false,sizeof(adj));
    range(m,i){
        string a,b;cin>>a>>b;
        int u=int(find(names,names+n,a)-begin(names));
        int v=int(find(names,names+n,b)-begin(names));
    ...
    do{
        bool ok=true;
        range(1,n,i){
            ok&=!adj[ord[i]][ord[i-1]];
        }
        if(ok){
            range(n,i)cout<<names[ord[i]]<<" ";
            cout<<"\n";
            return;
        }
