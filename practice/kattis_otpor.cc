...
int n;
double r[10];
deque<pair<char, vector<double>>> s;

double eval() {
    assert(len(s) && (s.back().first == '-' || s.back().first == '|'));
    double ans = 0;
    if (s.back().first == '-')
        ans = sum(s.back().second, x, x);
    else
        ans = 1. / sum(s.back().second, x, 1. / x);
    s.pop_back();
    if (len(s)) s.back().second.push_back(ans);
    return ans;
}
