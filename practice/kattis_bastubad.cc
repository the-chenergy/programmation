...
int n;
map<double, vector<array<double, 3>>> tes;
double sa, sb, sc;
...
void solve() {
    ...
    range(n) {
        double a, b, c, t;
        cin >> a >> b >> c >> t;
        tes[t].push_back({a, b, c});
        sa += a, sb += b, sc += c;
    }
    double pt = 0, ans = 0;
    each(tes, t, es) {
        trace(pt, t, sa, sb, sc, es);
        if (sa < 0) {
            double x = min(max(-sb / (2. * sa), pt), t);
            amax(ans, sa * x * x + sb * x + sc);
        } else {
            amax(ans, max(sa * pt * pt + sb * pt, sa * t * t + sb * t) + sc);
        }
        trace(ans);
        each(es, a, b, c) sa -= a, sb -= b, sc -= c;
        pt = t;
    }
    cout << ans;
}
