...
int ans[10009];
complex<double>x=0,d=exp(M_PI/6.*1.i);
map<array<ll,2>,int>vs;
int cs[6];
...
void solve() {
    ...
    range(2,10003,i){
        ...
        while(1){
            nx=x+d;
            np={ll(nx.real()*1E3+.5),ll(nx.imag()*1E3+.5)};
            trace("  try",np,arg(d)/M_PI*180);
            if(vs.count(np)){
                d*=exp(M_PI/3.*1.i);
                continue;
            }
        ...
        d*=-1;
        trace(i,np);
        bitset<6>pos;pos.set();
        range(6,p){
            auto na=x+d*exp((M_PI/3.*p)*1.i);
            auto nap=array{ll(na.real()*1E3+.5),ll(na.imag()*1E3+.5)};
            trace("  ",i,p,nap);
            if(vs.count(nap))pos.reset(vs[nap]);
        }
        int v=0;
        if(pos.count()==1){
            range(1,6,p)if(pos[p]){
                v=p;break;
            }
        }else{
            int mn=1E9;
