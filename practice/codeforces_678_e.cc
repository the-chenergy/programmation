// Solution to Another Sith Tournament, problem E from the Codeforces contest
// 678
// https://codeforces.com/contest/678/problem/E
// #dp #bitmask
//
// Let dp[R, x] represent the chance that x wins with R being the remaining set
// of opponents.
...
void solve() {
    ...
    dp[0][0]=1;
    for(int r=1;r<(1<<n)-1;r++){
        for(int y=n;--y;){
            if(!(r&1<<y))continue;
            amax(dp[r][0],p[0][y]*dp[r^1<<y][0]);
        }
        for(int x=n;~--x;){
            if(r&1<<x)continue;
            for(int y=n;~--y;){
                if(!(r&1<<y))continue;
                amax(dp[r][x],p[x][y]*dp[r^1<<y][x]+(1-p[x][y])*dp[r^1<<y][y]);
            }
        }
    }
    double ans=0;
    for(int x=n;~--x;)amax(ans,dp[((1<<n)-1)^1<<x][x]);
