// Solution to Mårten's DFS from the problem set Principles of Algorithmic
// Problem Solving
// (https://open.kattis.com/problems/martensdfs)
//
// We can use the claimed output to retrospectively figure out the order of the
// neighbors in the adjacency lists. More specifically, we can sort all
// vertices' neighbors according to where they show up in the claimed output.
// Then we can run an actual DFS to simulate the process and check if the output
// matches the claimed output.
//
// Since the graph is connected and the DFS shouldn't visit the same vertex more
// than once, the claimed output must have a length exactly N and include every
// vertex. Otherwise it's definitely invalid.
//
// O(N log N + E)
...
int n,m,ord[100009];
char ex[100009],seen[100009];
vi g[100009],l,r;

#define say(s)return (void)(cout<<s)

void dfs(int i){
    r.push_back(i);
    seen[i]=1;
    for(int j:g[i])if(!seen[j])dfs(j);
}

void solve() {
    // memset(ex,0,sizeof(ex[0])*n);
    ...
    for(int i=0;i<n;i++)sort(all(g[i]),[&](int a,int b){return ord[a]<ord[b];});
