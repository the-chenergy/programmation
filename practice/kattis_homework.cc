// Solution to Homework, a problem from ACPC 2018
// (https://open.kattis.com/homework)
// #dp
//
// Each character in s is either some ith character in s1 or jth character in
// s2, where i+j must equal to the index of that character in s. We can
// translate this into the following recurrence relation:
//     match(s1[i:], s2[j:]) := (s[i+j] == s1[i] and match(s1[i+1:], s2[j:])) or
//                              (s[i+j] == s2[j] and match(s1[i:], s2[j+1:]))
// The answer is given by match(s1[0:], s2[0:]).
...
void solve() {
    cin >> s >> a >> b;
    if (0 || !cin) exit(0);
    n = len(a), m = len(b), assert(n + m == len(s));
    dp[n][m] = true;
    for (int i = n; ~i; i--)
        for (int j = m; ~j; j--) {
            if (i == n && j == m) continue;
            if ((i == n || s[i + j] != a[i]) && (j == m || s[i + j] != b[j]))
                continue;
            if (i < n && s[i + j] == a[i]) dp[i][j] = dp[i + 1][j];
            if (j < m && s[i + j] == b[j]) dp[i][j] |= dp[i][j + 1];
            trace(i, j, i + j, s[i + j], a[i], b[j]);
        }
    cout << (dp[0][0] ? "yes" : "no");
}
