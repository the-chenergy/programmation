...
int h, w, dp[102][102][2][2];  // y, x, player, tar

int happy(int y, int x, int dy, int dx) {
    return (dy % 2 && dx % 2) * (y % 2 == x % 2 ? 1 : -1);
}

void solve(int /* case_id */) {
    cin >> h >> w;
    memset(dp, -64, sizeof(dp));
    range(h + 2, y) range(2, p) range(2, t) dp[y][w + 1][p][t] = 0;
    range(w + 2, x) range(2, p) range(2, t) dp[0][x][p][t] = 0;
    view(dp, 0, h + 2, 0, w + 2);
    range(1, h + 1, y) range(w, 0, -1, x) {
        int mxd = -1E9;
        range(x + 1, w + 2, nx) {
            trace("  ", 1, x, y, nx - x);
            int nm = happy(1, x, y, nx - x) + dp[y][nx][1][0];
            int ns = dp[y][nx][1][1];
            if (amax(mxd, nm - ns)) dp[y][x][0][0] = nm, dp[y][x][0][1] = ns;
        }
        trace(h, w, y, x, dp[y][x][0]);
        mxd = -1E9;
        range(y - 1, -1, -1, ny) {
            trace("  ", ny, x, y - ny, w + 1 - x);
            int nm = dp[ny][x][0][0];
            int ns = happy(ny + 1, x, y - ny, w + 1 - x) + dp[ny][x][0][1];
            if (amax(mxd, ns - nm)) dp[y][x][1][0] = nm, dp[y][x][1][1] = ns;
        }
        trace(h, w, y, x, dp[y][x][1]);
    }
    cout << dp[h][1][0][0] - dp[h][1][0][1];
}
