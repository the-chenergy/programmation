...
// Solves group 1 of The Apple Game
// This looks like a Grundy's Game variant and I solved it basically with
// Sprague-Grundy, using a table filling DP like in the Coin Problem.
// O(N**2). Need to think of an efficient way to calculate all the pair-wise xor
// or do something more clever
void solve() {
    cin>>n;
    if (0 || cin.fail()) exit(0);
    cout<<(w[n/2-2]^w[n-n/2-2]?"alf":"beata");
}
...
void solve_suite() {
    ...
    range(6,1003,i){
        ...
        mex.set(w[i-3]);
        mex.set(w[i-4]);
        if(i<=12)trace(i,i-3,i-4,w[i-3],w[i-4]);
        range((i-5)/2+1,j){
            int k=i-5-j;
            if(i<=12)trace(i,j,k,w[j],w[k],w[j]^w[k]);
            mex.set(w[j]^w[k]);
        }
        range(1009,j)if(!mex[j]){
            w[i]=j;
            break;
        }
    }
