...
int h, w, a;
char g[10][10];
map<int, map<int, char>> ng;

array<int, 2> rot(int y, int x) {
    complex<double> p(x * 2., y * 2.);
    auto q = p * exp(a / 180. * M_PI * 1.i);
    if (a % 90)
        q /= M_SQRT2;
    else
        q /= 2;
    return {int(round(q.imag())), int(round(q.real()))};
}

void solve() {
    cin >> h >> w;
    // if (0 || cin.fail()) _cout.setstate(_S_failbit), exit(0);
    range(h, y) range(w, x) cin >> g[y][x];
    cin >> a;
    ng.clear();
    int mnx = 0;
    range(h, y) range(w, x) {
    ...
    each(ng, y, row) {
        cout << nl, nl = "\n";
        range(mnx, row.begin()->first, x) cout << " ";
        string sep = "";
        each(row, x, c) cout << sep << c, sep = a % 90 ? " " : "";
    }
}
