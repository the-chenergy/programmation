...
void solve() {
    ...
    range(2,n+1,i){
        if(d[i]>1)continue;
        if(n<100||i%1000==1)trace(i);
        // range(i,n+1,i,j)d[j]=3;
        int b=2;
        range(i,n+1,i,j)a[j]=0;
        for(ll p=i;p<=n;p*=i,b++){
            if(n<100||i%1000==1)trace(i,p);
            range(p,n+1,p,j)a[j]++;
        }
        range(i,n+1,i,j){
            if(n<100||i%1000==1)trace(i,j);
            d[j]*=(a[j]+1);
        }
    }
    range(1,n+1,i)cout<<d[i]<<"\n";
}
