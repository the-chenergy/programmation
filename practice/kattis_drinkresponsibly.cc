...
void solve() {
    double m_, u_;
    cin >> m_ >> u_ >> d;
    m = int(m_ * 100 + .5), u = int(u_ * 60 + .5);
    if (0 || cin.fail()) _cout.setstate(_S_failbit), exit(0);
    names.resize(d), vol.resize(d), cost.resize(d);
    range(d, i) {
        int s, l;
        double c;
        string ls;
        cin >> names[i] >> s >> ls >> c;
        l = ls == "1/1" ? 60 : ls == "1/2" ? 30 : 20;
        vol[i] = s * l, cost[i] = int(c * 100 + .5);
    }
    // trace(m, u, d, names, vol, cost);
    memset(dp, false, sizeof(dp));
    // range(m + 1, x) range(d + 1, i) dp[x][0][i] = true;
    range(d + 1, i) dp[0][0][i] = true;
    range(d - 1, -1, -1, i) range(m + 1, x) range(u + 1, y) {
        range(1001, t) {
            if (x - cost[i] * t < 0 || y - vol[i] * t < 0) break;
            if ((dp[x][y][i] = dp[x - cost[i] * t][y - vol[i] * t][i + 1]))
                break;
        }
    }
    if (!dp[m][u][0]) {
    ...
    range(d, i) {
        range(1001, t) {
            if (x - cost[i] * t < 0 || y - vol[i] * t < 0) break;
            // trace(x, y, i, t, dp[x][y][i]);
            if (dp[x - cost[i] * t][y - vol[i] * t][i + 1]) {
                if (t) cout << names[i] << " " << t << "\n", printed = true;
                x -= cost[i] * t, y -= vol[i] * t;
                break;
            }
        }
    }
