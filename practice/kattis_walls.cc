...
int n;
double l,w,r;
complex<double>cs[30],wa[4];

#define ans(x) return void(cout<<(x)<<"\n")
...
void solve() {
    ...
    wa[0]={-l/2,0},wa[1]={l/2,0},wa[2]={0,-w/2},wa[3]={0,w/2};
    
    bool ok=true;
    range(4,i){
        ok&=any_of(cs,cs+n,selecting(c,norm(wa[i]-c)<=r*r));
    }
    if(!ok)ans("Impossible");
    
    range(n,c){
        if(all_of(wa,wa+4,selecting(x,norm(x-cs[c])<=r*r)))ans(1);
    }
    
    range(n,c)range(n,d)if(c!=d){
        range(1<<4,s)if(__builtin_popcount(s)==2){
            ok=true;
            range(4,i)if(s&(1<<i)){
                ok&=norm(wa[i]-cs[c])<=r*r;
            }
            if(!ok)continue;
            range(4,i)if(!(s&(1<<i))){
                ok&=norm(wa[i]-cs[d])<=r*r;
            }
            if(ok)ans(2);
        }
    }
