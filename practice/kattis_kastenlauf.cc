// Solution to Surely You Kastenlauf, a problem from the GCPC 2013
// (https://open.kattis.com/problems/kastenlauf)
// #graph #dfs #components
//
// May also be solved with a Union-Find with way less code.
...
void solve() {
    ...
    for(int i=0;i<n+2;i++){
        cin>>pos[i][0]>>pos[i][1];
        nei[i].clear();
        for(int j=0;j<i;j++)if(reach(i,j))nei[i].push_back(j),nei[j].push_back(i);
    }
    memset(vis,false,sizeof(vis));
    dfs={0},vis[0]=true;
    while(len(dfs)){
        auto u=dfs.back();dfs.pop_back();
        for(auto v:nei[u])if(!vis[v]){
            if(v==n+1){
                cout<<"happy";
                return;
            }
