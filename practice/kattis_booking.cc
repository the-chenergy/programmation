// Solution to Booking, a problem from the GCPC 2013
// (https://open.kattis.com/problems/booking)
// #intervals #dates #formatting
//
// Convert all dates into the endpoint list representation of intervals. Find
// the maximum stack height while sweeping across.
...
void solve() {
    ...
    ps.clear(), mnt = 1E18;
    for (int i = 0; i < n; i++) {
        char a[19], aa[19], b[19], bb[19];
        scanf(" %*s %s %s %s %s", a, aa, b, bb);
        string s(a), t(b);
        s.push_back(' '), s.append(aa), t.push_back(' '), t.append(bb);
        tm s_{}, t_{};
        strptime(s.c_str(), tf, &s_);
        ll x = ll(mktime(&s_));
        strptime(t.c_str(), tf, &t_);
        ll y = ll(mktime(&t_)) + c;
        assert(x < y || trace(case_id, i, x, y, s, t, s_.tm_year, s_.tm_mon,
                              s_.tm_mday, s_.tm_hour, s_.tm_min, t_.tm_year,
                              t_.tm_mon, t_.tm_mday, t_.tm_hour, t_.tm_min));
        amin(mnt, x);
        ps.push_back({x, 1}), ps.push_back({y, -1});
    }
    for (auto& [p, d] : ps) p -= mnt;
    sort(all(ps));
    trace(ps);
    int ans = 0, l = 0;
    for (auto [p, d] : ps) {
