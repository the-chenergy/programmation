// Solution to Peculiar Movie Preferences, problem D from the Codeforces Round
// 767 (Div. 2) https://codeforces.com/contest/1629/problem/D
//
// Just check the possibilities involving sequence lengths 1 or 2, i.e. either
//   (1) the current word is a palindrome, or
//   (2) there exists a previous word that can pair with current word to form a
//   palindrome.
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (len(t) == 1) {
            ...
            ok |= !!s.count(u);
            for (char c = 'A'; c <= 'z'; c++) ok |= !!s.count(u + c);
            s.insert(t);
        } else if (len(t) == 3) {
            ok |= t[0] == t[2];
            string u(t.rbegin(), t.rend());
            ok |= !!s.count(u) || !!s.count(u.substr(0, 2));
            s.insert(t);
        }
    }
    cout << (ok ? "YES" : "NO");
