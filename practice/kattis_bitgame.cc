...
int ask(int s) {
    ...
    printf("? %i\n", s ^ (1 << l));
    cin >> g[s];
    debug trace(s, s ^ (1 << l), g[s]);
    return g[s];
}

void mess() {
    return;  // Test data were less evil than expected
    while (len(unk) && ~g[unk.back()]) unk.pop_back();
    if (len(unk)) {
        int s = unk.back();
        unk.pop_back(), ask(s);
    }
...
int calc(int s) {
    ...
    int t = s << 1;
    t |= !(rand() & 2);
    return g[s] = !calc(t) || !calc(t ^ 1);
}

void solve() {
    // if (0 || cin.fail()) exit(0);
    srand(uint(time(nullptr)) ^ 420);
    memset(g, -1, sizeof(g));
    range(1 << l, 1 << (l + 1), i) unk.push_back(i);
    shuffle(all(unk), mt19937(uint(time(nullptr)) ^ 420));
    printf("! %s\n", calc(1) ? "alice" : "bob");
    trace(qs);
    range(1, (1 << (min(l, 4) + 1)) + 1, i) {
        bitset<5> _(i);
        trace(i, _, g[i]);
    }
    trace(qs);
...
void solve_suite() {
    debug freopen("./sandbox/i21.txt", "r", stdin);
