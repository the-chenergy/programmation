...
const int mxn = 13;
using B = bitset<DEBUG ? 3 : mxn>;
int n;
vi t[mxn];
double p[mxn], q[mxn], e[1 << mxn][mxn][mxn];

int next_shooter(int i, int r) {
    int x = r & ~((1 << (i + 1)) - 1);
    return __lg(x ? x & -x : r & -r);
}
...
void solve() {
    ...
    memset(e, 0, sizeof(e));
    range(n, i) range(n, s) e[1 << i][s][i] = 1;

    range(3, 1 << n, r) if (__builtin_popcount(r) >= 2) {
        range(n, s) if (r & (1 << s)) {
            t[s].clear();
            double mxe = 0;
            range(n, k) if (k != s && r & (1 << k)) {
                int nr = r ^ (1 << k);
                double ne = e[nr][next_shooter(s, nr)][s];
                if (abs(ne - mxe) < 1E-10)
                    t[s].push_back(k);
                else if (amax(mxe, ne))
                    t[s] = {k};
            }
            trace("target", B(r), s, t[s]);
        }
        range(n, s) if (r & (1 << s)) {
            range(n, i) if (r & (1 << i)) {
                double tq = 1;
                ...
                do {
                    ...
                    each(t[cs], k) {
                        int nr = r ^ (1 << k);
                        tp += tq * p[cs] * e[nr][next_shooter(cs, nr)][i];
                    }
                    e[r][s][i] += tp / len(t[cs]);
                    tq *= q[cs];
                    cs = next_shooter(cs, r);
                } while (cs != s);
                e[r][s][i] /= (1 - tq);
                trace("  expect", B(r), s, i, e[r][s][i]);
            }
        }
    }

    range(n, i) cout << 100 * e[(1 << n) - 1][0][i] << " ";
}
...
void solve_suite() {
    ...
    for (case_id = 1; case_id <= num_cases; case_id++) {
        debug cerr.clear(case_id == 04 ? _S_goodbit : _S_failbit);
