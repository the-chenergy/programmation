...
int p,d,n,ans[900],alive[900],na[900];
pair<int,vector<array<int,2>>>g[30][30],ng[30][30];

void solve(int /* case_id */) {
    cin>>p>>d>>n;
    range(p,i){
        int x,y;cin>>x>>y;
        alive[i]=true,g[y][x].first=i+1;
        adj4(dy,dx)g[y][x].second.push_back({dy,dx});
    }
    range(1,d+1,id){
        range(n,y)range(n,x)ng[y][x]=g[y][x];
        memcpy(na,alive,sizeof(na));
        range(n,cy)range(n,cx){
            if(!g[cy][cx].first)continue;
            int pi=g[cy][cx].first-1;
            if(!alive[pi])continue;
            ans[pi]=id;
            each(g[cy][cx].second,dy,dx){
                auto y=cy+dy,x=cx+dx;
                if(!~y||!~x||y==n||x==n){
                    na[pi]=false;
                    trace("  ",pi,"hit wall at",y,x);
                    continue;
                }
                if(ng[y][x].first){
                    int qi=ng[y][x].first-1;
                    assert(qi!=pi);
                    na[pi]=na[qi]=false;
                    trace("  ",pi,"hit",qi,"at",y,x);
                }
                ng[y][x]={g[cy][cx].first,{{dy,dx}}};
                trace(pi,"grew from",cy,cx,"into",y,x);
            }
            ng[cy][cx].second.clear();
        }
        range(n,y)range(n,x)g[y][x]=ng[y][x];
        memcpy(alive,na,sizeof(na));
    }
    range(p,i)cout<<(alive[i]?"ALIVE":to_string(ans[i]))<<"\n";
}
