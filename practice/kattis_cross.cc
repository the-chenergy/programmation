...
void pb(){
        range(9,y){
            range(9,x)cout<<(g[y][x]?to_string(g[y][x]):".");
            cout<<"\n";
        }
}
...
bool check(){
    ...
    range(0,9,3,sy)range(0,9,3,sx){
        s=0;
        range(sy,sy+3,y)range(sx,sx+3,x){
            if(g[y][x]){
                if(s&(1<<g[y][x]))return false;
                s|=1<<g[y][x];
            }
...
void solve() {
    ...
    if(!check()){
        // if(g[0][0]!=1){vector<ll>a(1E9,4);a[0]+=time(nullptr);}
        cout<<"ERROR\n";
        return;
    }
    bool go=true,went=false;
    while(go){
        go=false;
        range(1,10,d){
            ...
            range(9,y)range(9,x){
                if(g[y][x]!=d)continue;
                range(9,i)cr[i][x]=cr[y][i]=true;
                range(y/3*3,y/3*3+3,dy)range(x/3*3,x/3*3+3,dx)cr[dy][dx]=true;
                saw[y/3][x/3]=true;
            }
            range(0,9,3,sy)range(0,9,3,sx){
                int ey=-1,ex=-1,c=0,e=0;
                range(sy,sy+3,y)range(sx,sx+3,x){
                    if(!cr[y][x]&&g[y][x]==d)while(1);
                    if(!cr[y][x]&&g[y][x]!=d){
                        c++;
                        if(!g[y][x]){
                            ey=y,ex=x;
...
void solve_suite() {
    ...
    for (case_id = 1; case_id <= num_cases; case_id++) {
        // debug cerr.clear(case_id == 3 ? _S_goodbit : _S_failbit);
        // debug _cout.clear(case_id == 3 ? _S_goodbit : _S_failbit);
