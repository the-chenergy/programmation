...
const ll inf = 1E14;
int n, d;
ll c[1009], dp[2009][1009];
vi nei[1009];

/*
Min cost selecting exits for subtree rooted at u, with prev exit l steps away
f(l, u) = min(
    c[u] + sum(u->v | f(1, v)),
    sum(u->v | f(l+1, v),
    min(u->v | min(l+1 <= k <= 2d+1 |
        f(2d+3-k, v) + sum(u->x, x!=v | f(k, x))
    ))
)
*/
...
void dfs(int u, int p) {
    ...
    if (has) {
        range(2 * d + 1, 0, -1, k) {
            ll tot = 0;
            each(nei[u], v) if (v != p) tot += dp[2 * d + 3 - k][v];
            each(nei[u], v) if (v != p) {
                amin(dp[k - 1][u], tot - dp[2 * d + 3 - k][v] + dp[k][v]);
            }
            amin(dp[k - 1][u], dp[k][u]);
        }
    }
    range(2 * d + 2, l) {
        if (!has && l <= d) {
        ...
        ll tot = c[u];
        each(nei[u], v) if (v != p) tot += dp[1][v];
        amin(dp[l][u], tot);
        if (has && l < 2 * d + 1) {
            tot = 0;
            each(nei[u], v) if (v != p) tot += dp[l + 1][v];
            amin(dp[l][u], tot);
            trace("  ", u, p, l, tot);
        }
    }
    range(2 * d + 2, l) trace(u, p, l, dp[l][u]);
}
...
void solve() {
    ...
    range(2 * d + 2, l) range(n, u) dp[l][u] = inf;
    dfs(0, -1);

    ll ans = inf;
    range(d + 1, 2 * d + 2, l) amin(ans, dp[l][0]);
    cout << ans;
}
