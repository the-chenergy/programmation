// Solution to Matryoshka, a problem from the ICPC 2013 World Finals
// (https://open.kattis.com/problems/matryoshka)
// #array #dp #precompute
//
// Let's assume we knew the sets of dolls in the row, i.e. where each set starts
// and where it ends. Let cost(A[b:e]) be the minimum moves needed to solve the
// set A[b:e], or infinity if A[b:e] does not form a valid set. (A valid set
// should contain exactly the integers 1..(e-b).) This then follows
// ans(A[i:]) := min(j > i | cost(A[i:j]) + ans([j:]), the usual
// optimal-partition DP.
//
// Now all that's left is an efficient way to calculate ans(A[b:e]). Since any
// assembled set is eventually combined from two groups, and each group is built
// from combining two groups, we can let merge(A[l:r], m) be the moves needed to
// assemble A[l:m] and A[m:r]. You'll find that the pattern is, to merge two
// groups A and B, the number of moves needed is equal to the number of elements
// >= min(min(A), min(B)). You can use this formula to work out a clever way to
// precompute all values of merge(A[l:r], m) for all {l, r, m} in O(n**3). And
// so finally, cost(A[b:e]) :=
//    min(b < j < e | cost(A[b:j]) + cost(A[j:e]) + merge(A[b:e], j)),
// which is basically another DP.
//
// The overall running time should be some amortized O(n**3). It might look like
// O(n**5) but cost(A[i:j]) actually shouldn't be invoked that much -- only when
// A[i:j] actually forms a valid set and also leaves the rest of the array in a
// partition-able state.
...
int n, a[mxn], dp[mxn + 1], com[mxn][mxn + 1], mer[mxn][mxn + 1][mxn];
bitset<mxn> has;
vi pre;

int cost(int b, int e) {
    has.reset();
    range(b, e, i) {
        if (a[i] >= e - b || has[a[i]]) return inf;
        has[a[i]] = true;
    }
    ...
    range(b, e, i) {
        pre.clear();
        range(i + 1, e, j) {
            pre.insert(lower_bound(all(pre), a[j - 1]), a[j - 1]);
            int mn = 1E9, hc = 0;
            range(j + 1, e + 1, k) {
                if (a[k - 1] > pre[0]) hc++;
                amin(mn, a[k - 1]);
                mer[i][j][k] = hc + int(end(pre) - lower_bound(all(pre), mn));
            }
        }
    }

    range(2, e - b + 1, l) range(b, e - l + 1, i) range(i + 1, i + l, j) {
        amin(com[i][i + l], com[i][j] + com[j][i + l] + mer[i][j][i + l]);
    }
    return com[b][e];
}
...
void solve() {
    cin >> n;
    if (0 || !cin) exit(0);
    range(n, i) cin >> a[i], a[i]--;
    view(a, 0, n);

    memset(com, 63, sizeof(com));
    range(n, i) com[i][i + 1] = 0;

    memset(dp, 63, sizeof(dp)), dp[n] = 0;
    range(n - 1, -1, -1, b) range(b + 1, n + 1, e) if (dp[e] < inf) {
        int c = cost(b, e);
        if (c < inf) amin(dp[b], c + dp[e]);
    }

    cout << (dp[0] >= inf ? "impossible" : to_string(dp[0]));
}
...
void solve_suite() {
    ...
    for (case_id = 1; case_id <= num_cases; case_id++) {
        // debug cerr.clear(case_id <= 2 ? _S_goodbit : _S_failbit);
