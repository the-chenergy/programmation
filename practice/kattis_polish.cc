...
expr* read() {
    ...
    in >> op;
    if (isalnum(op.back())) {
        bool is = !!isalpha(op.back());
        return new expr{is, is, is ? op[0] : '.', is ? 0 : stoi(op), {}};
    }
    expr* l = read();
    expr* r = read();
    if (l->has || r->has) return new expr{true, false, op[0], 0, {l, r}};
    int a;
    if (op == "+")
        a = l->val + r->val;
    else if (op == "-")
        ...
        assert(op == "*" || trace(op)), a = l->val * r->val;
    delete l, delete r;
    return new expr{false, false, '.', a, {}};
}
