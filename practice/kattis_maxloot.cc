...
int n;
ll c,w[69],v[69],dp[1000009],ndp[1000009];
map<ll,ll>wv;
...
void solve_mm(){
    ...
    range(1,1<<(n/2),s){
        ll tw=0,tv=0;
        range(n/2,i)if((s>>i)&1)tw+=w[i],tv+=v[i];
        if(tw<=c)wv[tw]=max(wv[tw],tv);
    }
    ll p=0;
    each(wv,wx,vx)p=max(p,vx),vx=p;
    ll ans=0;
    range(1,1<<(n-n/2),s){
        ll tw=0,tv=0;
        range(n/2,n,i)if((s>>(i-n/2))&1)tw+=w[i],tv+=v[i];
        if(tw>c)continue;
        if(auto it=wv.upper_bound(c-tw);it!=wv.begin())tv+=prev(it)->second;
        ans=max(ans,tv);
    }
    cout<<ans;
}
...
void solve_dp(){
    ll tv=reduce(v,v+n,0LL);
    assert(tv<=len(dp));
    memset(dp,63,sizeof(dp[0])*(tv+1)),dp[0]=0;
    range(n-1,-1,-1,i){
        memcpy(ndp,dp,sizeof(dp[0])*(tv+1));
        range(tv+1,r)if(r-v[i]>=0)amin(ndp[r],dp[r-v[i]]+w[i]);
        memcpy(dp,ndp,sizeof(dp[0])*(tv+1));
    }
    range(tv,-1,-1,vx)if(dp[vx]<=c){
        cout<<vx;
        return;
    }
    assert(0);
...
void solve_suite() {
    ...
    // local cout.open("./sandbox/oe.txt");
    num_cases = 1 << 0;
    cin >> num_cases;
    // local num_cases = 1E9;
