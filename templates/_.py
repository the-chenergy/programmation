...
import array, bisect, collections, cmath, functools, heapq, itertools, inspect, io, math, operator, os, random, re, sys, time
...
def trace(*args):
    ...
    for i, x in enumerate(args):
        if i:
            eprint(end=', ')
        try:
            eprint(f'{x}: {eval(x, frame.f_globals, frame.f_locals)}', end='')
        except:
            eprint(x, end='')
    eprint()
