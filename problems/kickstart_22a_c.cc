...
void solve(int /* case_id */) {
    ...
    string s;
    cin >> n >> s;
    auto dp = vector(n + 1, vector(1 << 6, 0));
    for (int p = 0; p < (1 << 6); p++) dp.back()[p] = 1;
    for (int i = n - 1; i >= 0; i--) {
        for (int p = 0; p < (1 << 6); p++) {
            if (s[i] != '?' && s[i] - '0' != (p & 1)) continue;
            bitset<6> pb(p);
            if (i >= 4 && pb[0] == pb[4] && pb[1] == pb[3]) continue;
            if (i >= 5 && pb[0] == pb[5] && pb[1] == pb[4] && pb[2] == pb[3])
                continue;
            pb <<= 1;
            for (int l = 0; l < 2; l++) {
                pb[0] = l;
                dp[i][p] |= dp[i + 1][pb.to_ulong()];
            }
        }
    }
    for (int p = 0; p < (1 << 6); p++) {
