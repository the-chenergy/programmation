...
public:
    bool isPossible(int n, vector<vector<int>>& edges) {
        ...
        set<pii>oin,ein;
        range(n,i){
            if(len(adj[i])%2)oin.emplace(len(adj[i]),i);
            else ein.emplace(len(adj[i]),i);
        }
        trace(oin,ein);
        if(!len(oin))return true;
        if(len(oin)==2){
            auto[ix,x]=*begin(oin);
            auto[iy,y]=*next(begin(oin));
            if(!adj[x].count(y))return true;
            each(ein,in,u){
                if(!adj[u].count(x)&&!adj[u].count(y))return true;
            }
            return false;
        }else if(len(oin)==4){
            auto[ix,x]=*begin(oin);
            auto[iy,y]=*next(begin(oin));
            auto[iz,z]=*next(next(begin(oin)));
            auto[iw,w]=*next(next(next(begin(oin))));
            vi aa={x,y,z,w};
            range(1<<4,s)if(__builtin_popcount(s)==2){
                int a=__lg(s);
                int b=__lg(s^(1<<a));
                int c=__lg(s^15);
                int d=__lg((s^15)^(1<<c));
                trace(a,b,c,d,aa);
                if(!adj[aa[a]].count(aa[b])&&!adj[aa[c]].count(aa[d]))return true;
            }
            return false;
        }
        return false;
