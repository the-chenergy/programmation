...
// #define LOCAL 01  // If 0 or unset, disable trace, eprintf, and local blocks
...
class Solution {
public:
    vector<array<int,2>>qai;//query_val,ans_i
    vector<int>ans;
    map<int,vector<array<int,2>>>vps;//val->positions
    int h,w;
    map<int,ll>tot;
    vector<int> maxPoints(vector<vector<int>>& grid, vector<int>& queries) {
        enumerate(queries,i,x)qai.push_back({x,i});
        sort(all(qai));
        ans.resize(len(queries));
        
        h=len(grid),w=len(grid[0]);
        UF uf(h*w);
        range(h,y)range(w,x){
            vps[grid[y][x]].push_back({y,x});
            // tot[y*w+x]=1;//grid[y][x];
        }
        // trace(tot);
        ...
        auto it=vps.begin();
        each(qai,qv,ai){
            while(it!=vps.end()&&it->first<qv){
                each(it->second,y,x){
                    adj4(h,w,y,x,ny,nx){
                        local if(uf.sameSet(y*w+x,ny*w+nx))continue;
                        local trace(qv,ai,it->first,y,x,ny,nx);
                        if(grid[ny][nx]>=qv)continue;
                        // ll t1=tot[uf.find(y*w+x)],t2=tot[uf.find(ny*w+nx)];
                        uf.join(y*w+x,ny*w+nx);
                        // tot[uf.find(y*w+x)]=t1+t2;
                    }
                }
                it++;
            }
            // trace(qv,ai,tot);
            // ans[ai]=tot[uf.find(0)];
            ans[ai]=grid[0][0]<qv?uf.size(0):0;
        }
