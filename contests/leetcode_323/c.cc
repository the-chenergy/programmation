...
class Allocator {
public:
    map<int,pair<int,bool>>fl;
    // map<int,int>btm;
    map<int,vector<int>>mtb;//b,s
    Allocator(int n) {
        fl[0]={n,true};
    }
    
    int allocate(int size, int mID) {
        int nb=-1,nbs=-1,ans=-1;
        for(auto it=fl.begin();it!=fl.end();it++){
            if(it->second.first>=size&&it->second.second){
                if(it->second.first>size){
                    nb=it->first+size,nbs=it->second.first-size;
                }
                ans=it->first;
                // fl.erase(it);
                it->second.first=size;
                it->second.second=false;
                mtb[mID].push_back(it->first);
                if(nb>=0)fl[nb]={nbs,true};
                return ans;
            }
        }
        return -1;
    ...
    int free(int mid) {
        int ans=0;
        each(mtb[mid],i){
            auto it=prev(fl.upper_bound(i));
            ans+=it->second.first;
            it->second.second=true;
            if(next(it)!=fl.end()&&next(it)->second.second){
                it->second.first+=next(it)->second.first;
                fl.erase(next(it));
            }
            if(it!=fl.begin()&&prev(it)->second.second){
                it=prev(it);
                it->second.first+=next(it)->second.first;
                fl.erase(next(it));
            }
