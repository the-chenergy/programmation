...
# https://www.hackerrank.com/contests/lpc-2022/challenges/lpc2022-follow-the-current
def solve(case_id):
    l = int(input())
    ...
    for y in range(h):
        for x in range(w):
            s = input()
            if s == 'Land' or s == 'Land ' or s == 'Land  ':
                # Trailing whitespaces >_< Really, Lucid?
                g[y][x] = 1
            else:
                s = s.split()[1]
                g[y][x] = s[0].lower()
    ...
    def f(y, x, d, t):
        ...
        if g[y][x] == 'w':
            ...
            dy = 1
        return .5 * f((y + dy) % h, (x + dx) % w, (dy, dx), t + 1) + .5 * f(
            (y + d[0]) % h, (x + d[1]) % w, d, t + 1)
    ...
    if g[y][x] == 1:
        ...
        print(round(f((y + dy) % h, (x + dx) % w, (dy, dx), 0) * 100))
