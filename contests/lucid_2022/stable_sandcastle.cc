...
// https://www.hackerrank.com/contests/lpc-2022/challenges/lpc2022-stable-sandcastle
void solve(int /* case_id */) {
    int n;
    cin >> n;
    vector<int> hs(n + 6, 0);
    for (int i = 0; i < n; i++) cin >> hs[i + 3];
    bool ok = 1;
    for (int i = 3; i < n + 3; i++) {
        ok &= abs(hs[i] - hs[i - 1]) <= 4;
        ok &= abs(hs[i] - hs[i - 2]) <= 4;
        ok &= abs(hs[i] - hs[i - 3]) <= 4;
        ok &= abs(hs[i] - hs[i + 1]) <= 4;
        ok &= abs(hs[i] - hs[i + 2]) <= 4;
        ok &= abs(hs[i] - hs[i + 3]) <= 4;
    }
