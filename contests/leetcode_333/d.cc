...
class Solution {
  public:
    string findTheString(vector<vector<int>>& a) {
        ...
        ans.back() = 1;
        for (int j = n - 2; j >= 0; j--) {
            if (a.back()[j] > 1) return "";
            if (a.back()[j]) ans[j] = ans.back();
        }
        for (int i = n - 2, mx = 2; i >= 0; i--) {
            if (!ans[i]) ans[i] = mx++;
            if (ans[i] > 26) return "";
            for (int j = i - 1; j >= 0; j--) {
                if (a[i][j] && a[i][j] != 1 + a[i + 1][j + 1]) return "";
                if (a[i][j]) ans[j] = ans[i];
            }
        }
        map<char, char> ord;
        for (int i = 0; i < n; i++) {
            if (auto it = ord.find(ans[i]); it != end(ord))
                ans[i] = it->second;
            else
                ans[i] = ord[ans[i]] = char('a' + len(ord));
        }
        return ans;
    }
};
...
int main() {
    ...
    auto $t = [&](vector<vi> a) {
        cout << "Case #" << ++case_id << ": ";
        cout << $s.findTheString(a);
        cout << "\n";
    };

    $t({{4, 0, 2, 0}, {0, 3, 0, 1}, {2, 0, 2, 0}, {0, 1, 0, 1}});
    $t({{4, 3, 2, 1}, {3, 3, 2, 1}, {2, 2, 2, 1}, {1, 1, 1, 1}});
    $t({{4, 3, 2, 1}, {3, 3, 2, 1}, {2, 2, 2, 1}, {1, 1, 1, 3}});
}
#endif
